...
from .config_validation import validate_production
from .db import get_db
from .memory import get_memories_for_user, save_memory_for_user, delete_memory_for_user
from .metrics import snapshot
from .models import AuditLog, User
from .rate_limit import rate_limit, allow as rate_allow
from .readiness import check_database, check_redis
...
async def request_confirmation(action: str, user: User = Depends(current_user)):
...
async def use_confirmation(action: str, token: str, user: User = Depends(current_user)):
...
async def create_conversation_api(title: str = "محادثة جديدة", user: User = Depends(current_user)):
...
async def get_conversation_api(conversation_id: str, user: User = Depends(current_user)):
...
async def create_realtime_session_api(conversation_id: str, user: User = Depends(current_user)):
...
async def close_realtime_session_api(session_id: str, user: User = Depends(current_user)):
...
async def readiness():
    ...
    if missing:
        return {"ready": False, "missing_configuration": missing}
    try:
        ...
        db.add(
            AuditLog(
                ...
                metadata_json=json.dumps(
                    {"risk": risk, "arguments": payload.arguments},
                    ensure_ascii=False,
                )[:5000],
            )
        )
...
async def realtime_session(request: Request, user: User = Depends(current_user)):
